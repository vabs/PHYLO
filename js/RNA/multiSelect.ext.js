	...
	$.multiSelect = {
		...
		activeRNA : function() {
			...
			$("#game").unbind().dblclick(function(e) {
				e = $.events.getMultiSelectFingerPosRNA(e); //fingerPosRNA
				self.startTheEvents(e);								
			});	
		},
		startTheEventsRNA : function(e) {
			var self = this;
			var test = document.getElementById("gameBoardRNA");	//gameBoardRNA
		...
		captureRNA : function() {
			...
			var posL = test.scrollLeft;
			var box = {
				X: parseInt($("#selectBox").css("left").replace(/px/,"")) - (173-posL),//adjust with scroll (posL)
			...
			};	//display wrong capture correct
			var select = {
				X: $.phylo.seqLen*36,											//extend X based on seqLen to cover longer board
				Y: 1000,
				H: -1,
				W: -1,
			}
			var list = [];
			var list2 = [];
			$(".current > .sequenceRNA").each(function() {			//sequenceRNA
				var self = this;
				var row = parseInt($(this).parent().attr("id").replace(/row/,""));
				var curr = {
					X: parseInt($(this).css("left").replace(/px/,"")),
					Y: 34*row,
					H: 29,		//fixed height and weight since css is triangle now
					W: 29
				};
				if(
			...
			$.events.touchRNA("#chosenArea", {
				start: function(e) {
					var offsetX = e.pageX - select.X;
					$.events.touchRNA(document,{		//touchRNA
						move: function(e) {
							...
							$("#chosenArea").hide();
							$.physics.snap("mutli");
							var score = $.fitch.scoreRNA();	//scoreRNA
							$.board.score(score);
							$.board.stats();
